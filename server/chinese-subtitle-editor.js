...
export function parseChineseSubtitleDocument(content) {
  ...
  const cues = blocks.map((block, index) => {
    ...
    if (!text) {
      throw new Error(`第 ${number} 条字幕正文不能为空。`);
    }
